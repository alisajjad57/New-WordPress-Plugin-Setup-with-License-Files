<?php
if( !class_exists('Ld_Ceus_License') ) {
	class Ld_Ceus_License {
		/**
		...
		public function __construct( $plugin_name, $version ) {
			...
			$this->license_option_prefix = 'learndash_ceus';
			$this->version = $version;
			$this->page_tab = ( isset($_GET['tab']) && !empty($_GET['tab']) ) ? $_GET['tab'] : 'license';

			$this->setting_menu_slug = 'learndash-ceus';
			$this->options = []; 			//Ld_Ceus_Helper::get_settings();
		}
		...
		function plugin_updater() {
			...
			// setup the updater
			$edd_updater = new Ld_Ceus_Updater( $this->api_url, LEARNDASH_CEUS_FILE,
				array(
					'version' => $this->version, // current version number
					'license' => $license_key,             // license key (used get_option above to retrieve from DB)
					'item_id' => null,       // ID of the product
					'item_name' => urlencode(LEARNDASH_CEUS_PLUGIN_NAME),    // Name of the product
		...
		function activate_license() {
			...
			if( isset( $_POST["{$this->license_option_prefix}_license_activate"] ) ) {
				...
				$api_params = array(
					...
					'license'    => $license,
					'item_name'  => urlencode( LEARNDASH_CEUS_PLUGIN_NAME ), // the name of our product in EDD
					'url'        => home_url()
				);
					...
					if ( false === $license_data->success ) {
						...
						switch( $license_data->error ) {
							...
							case 'item_name_mismatch' :
								...
								$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), LEARNDASH_CEUS_PLUGIN_NAME );
								break;
		...
		function check_license() {
			...
			$api_params = array(
				...
				'license' => $license,
				'item_name' => urlencode( LEARNDASH_CEUS_PLUGIN_NAME ),
				'url'       => home_url()
			);
		...
		function admin_notices() {
			...
			?>
            <div class="notice notice-error is-dismissible"><p><?php echo sprintf(__('Your license key for <strong>%s</strong> is invalid or expired. Please go to the <a href="%s">License</a> page to fix this issue.'), $plugin_data['Name'], $license_setting_url);?></p></div>
            <?php
		}
